...
liza({
    pattern: "liza",
    alias: "liza",
    desc: "Interact with ChatGPT using the MD Api.",
    category: "ai",
    react: "🤖",
    use: ".liza <your query>",
    filename: __filename,
    ...
    try {
        ...
        if (!q) return reply("⚠️ Please provide a query for Liza Ai.\n\nExample:\n.liza What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the Liza API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/zu92j8.jpg'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Liza's Response:*\n\n${gptResponse}`;
...
liza({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/zu92j8.jpg'; // Replace with a valid image URL
